...
contract TheCrayBongs is ERC721Enumerable, Ownable {
    ...
    ) ERC721("Crazy Bongs Token", "CrazyBongs") {
        ...
        validatePurchasePrice(count)
    {
        ...
        require(treasury != address(0), "Crazy Bongs: treasury not set");
        require(tokenPrice > 0, "Crazy Bongs: token price not set");
        require(count > 0  && count <= maxPurchaseCount, "Crazy Bongs: invalid count");
        require(_nextTokenId + count <= maxSupply, "Crazy Bongs: max supply exceeded");
        require(count == tokenURIs_.length, "Crazy Bongs: mint count does not match the token count");
